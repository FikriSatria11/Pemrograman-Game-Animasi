...
public class heroBerjalanScript : MonoBehaviour
{
   Animator anim;
	Animator Animation;
	public int Speed;
    ...
    {   
		...
		if (Input.GetKey (KeyCode.I)) { // Fungsi untuk mendapatkan kode Keyboard D.
			transform.Translate (Vector2.up * Speed * Time.deltaTime); //Fungsi untuk gerakan karakter ke kanan sesuai dengan kecepatan input.
			Animation.SetBool ("Walk Pocong", true); // Fungsi untuk menyalakan animasi berjalan.
		} else if (Input.GetKey (KeyCode.I) && Input.GetKey (KeyCode.Space)) { // Fungsi jika kode Keyboard yang di gunakan D dan Spasi maka tetap menerapkan fungsi gerak ke kanan.
			transform.Translate (Vector2.up * Speed * Time.deltaTime); //Fungsi untuk gerakan karakter ke kanan sesuai dengan kecepatan input.
			Animation.SetBool ("Walk Pocong", true); // Fungsi untuk menyalakan animasi berjalan.
		} else { // Fungsi jika kondisi yang di hadapi selain Keyboard D dan Keyboard D+Spasi.
			Animation.SetBool ("Walk Pocong", false); // Fungsi untuk mematikan animasi berjalan.
		...
		if (Input.GetKey (KeyCode.K)) { // Fungsi untuk mendapatkan kode Keyboard D.
			...
			Animation.SetBool ("Walk Pocong", true); // Fungsi untuk menyalakan animasi berjalan.
		} else if (Input.GetKey (KeyCode.K) && Input.GetKey (KeyCode.Space)) { // Fungsi jika kode Keyboard yang di gunakan D dan Spasi maka tetap menerapkan fungsi gerak ke kanan.
			transform.Translate (Vector2.down * Speed * Time.deltaTime); //Fungsi untuk gerakan karakter ke kanan sesuai dengan kecepatan input.
			Animation.SetBool ("Walk Pocong", true); // Fungsi untuk menyalakan animasi berjalan.
		} else { // Fungsi jika kondisi yang di hadapi selain Keyboard D dan Keyboard D+Spasi.
			Animation.SetBool ("Walk Pocong", false); // Fungsi untuk mematikan animasi berjalan.
